...
class UltraDirector:
    ...
    def update_environment(self, env_state):
        """
        Updates the internal environment state from sensor fusion.
        """
        self.env_state = env_state
        # Update flow field obstacles if they exist in state
        if 'lidar_obstacles' in env_state:
             # This allows flow field to be always up to date even between plans
             pass 
    ...
    def plan(self, user_text, start_pos, subject_pos, obstacles):
        ...
        # Use fused obstacles if not provided
        if obstacles is None and self.env_state:
             obstacles = self.env_state.get('lidar_obstacles', [])
    ...
    def plan_shot(self, *args, **kwargs):
        """Alias for plan() to match Director Core expectations."""
